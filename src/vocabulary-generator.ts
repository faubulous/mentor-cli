...
export class VocabularyGenerator {
    public readonly indexFileName = "index.ts";

    public readonly sourceFileName = "src.ts";
     ...
     * The predicates that are used to describe a subject.
     */
    public get supportedDescriptionPredicates(): string[] {
        return Array.from(this._descriptionPredicates);
    }
     ...
     */
    private getNamespaceUri(uri: string): string {
     ...
     */
    private async _serializeIndex(path: string, modules: string[]): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            const file = join(path, this.indexFileName);
    ...
    private async _writeSourceIndex(path: string): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            const file = join(path, this.sourceFileName);
            const stream = fs.createWriteStream(file);
     ...
     * @param pattern Path or glob pattern of the directory to parse.
    ...
    public async parseDirectory(pattern: string, createIndex: boolean = false, createSourceIndex: boolean = false): Promise<string[]> {
        ...
        for (let file of files) {
            ...
            if (this._supportedExtensions.has(ext)) {
                result.push(await this.parseFile(file));
            }
        };
        ...
        if (createIndex) {
            ...
            result.push(await this._serializeIndex(directory, result));
        }

        if (createSourceIndex) {
            const directory = dirname(pattern);

            result.push(await this._writeSourceIndex(directory));
        }
